...
BaseItemDto _movie(String id, String name, String played) => BaseItemDto(
      ...
      type: 'Movie',
      userData:
          UserItemData(lastPlayedDate: _d(played), playbackPositionTicks: 1),
    );

final _resume = [
  _movie('inception', 'Inception', '2026-07-26T02:06:00Z'),
  _ep('csm-e2', 'Chainsaw Man', "Meowy's Whereabouts", '2026-04-18T02:49:21Z'),
  _ep('tz-valley', 'The Twilight Zone', 'Valley of the Shadow',
      '2025-09-07T15:25:23Z'),
  _ep('tj-brothers', 'Tom and Jerry', 'The Brothers Carry-Mouse-Off',
...
final _finished = [
  _ep('elr-13', 'Everybody Loves Raymond', "Debra's Sick",
  ...
  _ep('hm-22', 'The Honeymooners', 'Here Comes the Bride',
  ...
  _ep('tz-s3e29', 'The Twilight Zone', "Four O'Clock",
      '2026-03-20T11:43:04Z'),
  _ep('inv-3', 'INVINCIBLE', 'I Gotta Get Some Air', '2026-03-20T01:40:50Z'),
];

void main() {
  final merged = mergeContinueWatching(
      resume: _resume, nextUp: _nextUp, finished: _finished);
  final ids = merged.map((e) => e.id).toList();

  test('the show watched last night leads the row', () {
    expect(ids.first, 'elr-16',
        reason: 'Raymond was absent from Continue Watching and ninth in Next '
  ...
  group('Remove', () {
    ...
    test('clearing the resume point alone brings the show back', () {
      final after = mergeContinueWatching(
          resume: resumeAfterClear, nextUp: _nextUp, finished: _finished);
      ...
      final after = mergeContinueWatching(
        ...
        nextUp: [
          for (final e in _nextUp)
            if (e.id == 'csm-e5') _ep('csm-e6', 'Chainsaw Man', 'Kill Denji') else e
        ],
  ...
  test('removals survive a restart, per account', () {
    const prefs = Prefs(continueWatchingDismissed: {
